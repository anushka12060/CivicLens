 ...
 *              page key ("report" | "map" | "board" | "ward" | "officer") to
 *              deep-link directly to a specific page. Defaults to "report".
 */
 ...
 *
 * `icon` is optional: when present, a Bootstrap Icon is rendered before the
 * label in the desktop and mobile menus (the footer intentionally stays
 * icon-free to match its plain-text style).
 */
const NAV_ITEMS = [
  ...
  { label: "Officer Portal", page: "officer" },
] as const;
...
const LANDING_STYLES = `
  ...
  .cl-footer-grid { display: grid; grid-template-columns: 0.85fr 1.15fr; align-items: center; gap: 24px; }
  .cl-footer-brand { display: flex; align-items: center; gap: 12px; margin-bottom: 16px; }
  .cl-footer p { color: #64748B; line-height: 1.8; }
  .cl-footer-links { display: flex; justify-content: flex-end; gap: 18px; flex-wrap: nowrap; white-space: nowrap; }
          ...
          {NAV_ITEMS.map((item) => (
            <li key={item.page}>
              <button className="cl-nav-link" onClick={() => handleNav(item.page)}>
                {"icon" in item && (
                  <i className={`bi ${item.icon}`} style={{ marginRight: "6px" }} aria-hidden="true" />
                )}
                {item.label}
              </button>
            </li>
